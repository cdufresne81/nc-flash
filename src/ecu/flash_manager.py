...
class FlashManager:
    ...
    def _set_state(self, new_state: FlashState) -> None:
        ...
        if self._state in _TRANSITIONS and new_state not in _TRANSITIONS[self._state]:
            logger.error(
    ...
    def _archive_rom(self, rom_data: bytes, archive_dir: str) -> str:
        """
        Save a timestamped backup of the flashed ROM.

        Creates the archive directory if it doesn't exist.
        Filename format: flash_YYMMDD_HHMM.bin (matches romdrop's timestamp format).
        ...
        filename = f"flash_{timestamp}.bin"
        filepath = archive_path / filename

        filepath.write_bytes(rom_data)
        logger.info(f"ROM archived to {filepath}")
        return str(filepath)
    ...
    def flash_rom(
        ...
        Args:
            ...
            archive_dir: Optional directory to save a timestamped ROM backup after flash
        ...
        try:
            self._flash_rom_inner(
                rom_data, ROM_FLASH_START_MIN, progress_cb, archive_dir
            )
    ...
    def dynamic_flash(
        ...
        Compares rom_data against archive_rom_data to determine the
        minimal flash region, reducing flash time significantly.

        Args:
            rom_data: New ROM data to flash
            archive_rom_data: Current/archived ROM data for comparison
            progress_cb: Optional progress callback
            archive_dir: Optional directory to save a timestamped ROM backup after flash

        Raises:
            ROMValidationError: ROMs are identical or validation fails
            FlashError: Flash procedure failed
        """
        if not SECURE_MODULE_AVAILABLE:
            raise SecureModuleNotAvailable()
        ...
        try:
            self._flash_rom_inner(rom_data, flash_start, progress_cb, archive_dir)
        except FlashAbortedError:
            self._set_state(FlashState.ABORTED)
            self._notify(progress_cb, "Flash aborted by user")
            raise
    ...
    def _flash_rom_inner(
        ...
        if archive_dir:
            try:
                archived = self._archive_rom(bytes(rom_buf), archive_dir)
                self._notify(callback, f"ROM archived: {archived}", percent=94.0)
            except Exception as e:
                logger.warning(f"ROM archive failed (non-fatal): {e}")
