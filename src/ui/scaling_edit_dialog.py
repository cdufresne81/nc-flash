...
# Keep the old class for backward compatibility (single scaling edit)
class ScalingEditDialog(QDialog):
    """Dialog for editing a single scaling's properties (legacy)"""
    ...
    def _setup_ui(self):
        ...
        # Use ScalingTab for the form
        self.tab = ScalingTab(self.scaling, self.scaling_name)
    ...
    def _save(self):
        ...
        if self.tab.validate():
            self.accept()

    def get_values(self) -> dict:
        """Get edited values as a dictionary"""
        return self.tab.get_values()
