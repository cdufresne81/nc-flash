...
class _LogSyncWorker(QObject):
    ...
    def run(self):
        try:
            ...
            from src.ecu.wican_config import peek_datalog_client
            ...
            # While we monopolize the device's single-task httpd, an ECU
            # session's keepalive ticks time out by construction — quiet them
            # for the transfer window (logging-only; peek never creates).
            datalog = peek_datalog_client(host, **kwargs)
            if datalog is not None:
                datalog.set_bulk_transfer(True)
            try:
                ...
                if datalog is not None:
                    datalog.set_bulk_transfer(False)
