"""WiCAN device-utility button gating: Download Logs + Live Datalog.

Both buttons are WiCAN device utilities (pure HTTP / raw-TCP to the adapter,
no ECU session). With Tactrix/J2534 selected they must be HIDDEN entirely — not
just disabled. Utilities and ECU operations never mix: while either utility
runs, every ECU action locks (with an explanatory tooltip) and the utilities
lock each other — only the Live Datalog toggle stays enabled while streaming,
so it can be stopped. Both are held off while an ECU operation is busy.
...
UTILITY_LOCK_TIP = "Stop the WiCAN trip-log download / live datalog first"
...
def _fake_window(
    ...
    main_window = SimpleNamespace(
        ...
        wican_live_datalog=SimpleNamespace(is_running=live_running),
    ...
    return SimpleNamespace(
        ...
        _btn_live_datalog=MagicMock(),
        _btn_connect=MagicMock(),
        _live_datalog_status=MagicMock(),
...
def test_hidden_and_disabled_with_j2534_adapter():
    ...
    fake._btn_live_datalog.setVisible.assert_called_with(False)
    fake._btn_live_datalog.setEnabled.assert_called_with(False)
...
def test_visible_and_enabled_with_wican_adapter():
    ...
    fake._btn_live_datalog.setVisible.assert_called_with(True)
    fake._btn_live_datalog.setEnabled.assert_called_with(True)
    fake._btn_live_datalog.setText.assert_called_with("Live Datalog")
...
def test_download_disabled_but_visible_while_sync_running():
    ...
    fake._btn_download_logs.setVisible.assert_called_with(True)
    fake._btn_download_logs.setEnabled.assert_called_with(False)
    # Mutual exclusion: a running trip-log sync also locks the live toggle.
    fake._btn_live_datalog.setEnabled.assert_called_with(False)


def test_live_datalog_stays_enabled_and_relabeled_while_streaming():
    fake = _fake_window(adapter="wican", live_running=True)
    _update(fake)
    # The toggle stays enabled while streaming so it can be stopped, and the
    # label reflects the run state; Download Logs locks (mutual exclusion).
    fake._btn_live_datalog.setVisible.assert_called_with(True)
    fake._btn_live_datalog.setEnabled.assert_called_with(True)
    fake._btn_live_datalog.setText.assert_called_with("Stop Live Datalog")
    fake._btn_download_logs.setEnabled.assert_called_with(False)
...
def test_ecu_ops_locked_while_sync_running():
    # Utilities and ECU operations never mix: a running trip-log download
...
def test_ecu_ops_locked_while_live_datalog_running():
    fake = _fake_window(adapter="wican", live_running=True, connected=True)
    _update(fake)
    fake._btn_read_dtcs.setEnabled.assert_called_with(False)
    fake._btn_clear_dtcs.setEnabled.assert_called_with(False)
    fake._btn_flash_current.setEnabled.assert_called_with(False)
    fake._btn_flash_current.setToolTip.assert_called_with(UTILITY_LOCK_TIP)
    # The live toggle itself stays usable — it is how the stream is stopped.
    fake._btn_live_datalog.setEnabled.assert_called_with(True)


def test_ecu_ops_unlocked_when_no_utility_runs():
    # Control for the two tests above: the same connected window with no
    # utility running leaves connection-gated ops (DTCs) enabled — proving
    # they exercise the utility lock, not the no-connection gate.
...
def test_connect_locked_while_utility_runs_when_disconnected():
    # Connecting mid-stream would re-park the bus and starve the live trip of
    # records; while DISCONNECTED the Connect button follows the utility lock.
    fake = _fake_window(adapter="wican", live_running=True)
    _update(fake)
