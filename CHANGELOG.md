...
### Fixed
- **CI: flaky WiCAN transport test on the macOS runner** — `test_ecu_wican_transport.py::TestChannelPriming::test_flush_drains_pending_frames` held its mock-server socket open with a fixed `sleep(1.5)` that could expire mid-`receive_message()` on a slow runner, so the client correctly reported "socket closed by peer" and the test failed (only on macOS/3.12; ubuntu/3.10 + local passed). The handler now holds the socket open until the client disconnects, removing the timing dependency. Test-only — no product/behaviour change.
